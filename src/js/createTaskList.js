...
filterTask () {
    if (this.inputFilterEl.value == null) {
        ...
        this.filterArray = this.taskListState.arrayTask.filter(item => item.startsWith(this.inputFilterEl.value.toLowerCase()));
...
saveTask(event) {
    if (event.code === 'Enter') {
        ...
        if (this.inputFilterEl.value != '') {
            if (!this.taskListState.arrayTask.includes(this.inputFilterEl.value.toLowerCase())) {
                this.taskListState.arrayTask.push(this.inputFilterEl.value.toLowerCase());
                this.addTasks(this.taskListState.arrayTask);
            ...
            this.inputFilterEl.setAttribute('placeholder', 'Ошибка! Вы не ввели задачу!');
        }  
    } 
}
